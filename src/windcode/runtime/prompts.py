    ...
    *,
    ...
    if memory_enabled:
        memory_policy = (
            ...
            "工具失败、未调用或仅计划保存时不得声称写入成功。若本次任务产生了经过验证、"
            "可跨任务复用的工程经验, 可由你自行判断并调用 memory_write(kind=experience), "
            '无需用户额外输入"记住"; 不得保存普通检查结果、临时状态、推测或原始工具输出。'
            "稳定的用户身份和长期偏好会在本轮结束后由系统自动"
            ...
            "更新已有经验时, 先调用 memory_search 或 memory_get 定位唯一 ID, 再调用 "
            "memory_update 提交完整的新内容; 不得新增替代记录或删除旧记录。"
            "根据完整对话上下文判断用户是否要删除或忘记某条记忆, 包括承接上文对象的简短确认; "
            "仅在删除意图明确时, 才能在定位唯一 ID 后调用 memory_delete; "
            "只有工具返回 deleted 后才能声称删除成功。"
            "长期记忆的查询、写入、更新和删除只能使用 memory_* 工具; 不得使用 glob、grep、"
            "read_file、shell、SQLite 或直接操作数据库和记忆文件。"
