...
class SpawnSubagentsTool:
    ...
    description = (
        "Create bounded temporary subagents that run in the background. Researchers and "
        "verifiers are read-only and return their findings to the parent; use a worker/write "
        "task only when that child must edit files. This tool creates independent tasks, not "
        "coordinated teamwork; use collaborate_subagents when multiple participants must divide "
        "work, exchange multi-round feedback or rebuttals, and receive an independent synthesis. "
        "Completion results are delivered automatically as inbound messages — you do NOT need to "
        "call wait_subagents for read tasks. Continue with other work; when a subagent finishes "
        "its result will appear as a system notification in your next turn. Use wait_subagents "
        "only when you explicitly need to block for write-task commits before proceeding. "
        "Write tasks require a clean Git parent and use an isolated Worktree based on its current "
        "HEAD; a dirty or unsupported parent rejects the write batch while read tasks remain "
        "available. Never stash, commit, revert, move, or discard parent changes automatically. "
        "Omit allowed_tools unless a narrower tool set is required. Declare "
