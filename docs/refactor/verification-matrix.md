...
| 质量 | Ruff、Pyright、pytest、build | Python 3.11/3.12: 601 passed/3 skipped；其余门禁通过 | 全绿且测试纳入 Git/CI |
| 架构 | AST 依赖、参数、复杂度脚本 | 顶层/模块 SCC 与两类反向依赖均为 0；构造参数最大 11；`AgentLoop` 7；`start_run` C901 10；child create C901 3 | 达到硬阈值 |
