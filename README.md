...
当前版本是经过完整自动化验收的纵向 MVP，重点保证单 Agent 编码闭环、用户控制和故障恢复。项目暂不包含多智能体编排、MCP、Skills、浏览器控制、Web 界面、IDE 集成和远程执行后端。
...
| `protocol` | 必填 Provider 字段 | 说明 |
| --- | --- | --- |
| `anthropic_messages` | `model`、`api_key_env` | Anthropic Messages |
| `openai_responses` | `model`、`api_key_env` | OpenAI Responses |
| `openai_compatible` | `model`、`api_key_env`、`base_url` | OpenAI-compatible Chat Completions |
...
使用 `--resume SESSION_ID` 或 `/resume SESSION_ID` 可恢复现有会话。系统会从当前事件树头节点恢复对话消息、工具状态和模型上下文。
...
未知字段会产生配置错误。API Key 只从 `api_key_env` 指向的环境变量读取，不应直接写入 TOML。

## 常见问题

### 启动后提示 `no model transport is configured` 怎么办？

确认 `.windcode/config.toml` 中存在 `providers` 和 `primary_provider`，并确保 `api_key_env` 指向的环境变量已经设置。可以先执行：
...
启动时使用 `--model`，或在空闲状态使用 `/model MODEL`。自动故障切换只会使用配置中的 `fallback_chain`，不会静默选择未配置模型。
