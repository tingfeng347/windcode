...
class CommandApp(App[None]):
    ...
    def on_chat_input_skill_menu_update(self, event: ChatInput.SkillMenuUpdate) -> None:
        menu = self.query_one(CommandMenu)
        skills = (
            SkillDefinition("code-review", "Review code"),
            SkillDefinition("test-runner", "Run tests"),
        )
...
async def test_slash_filters_and_tab_completes_selected_command(tmp_path: Path) -> None:
    ...
    async with app.run_test() as pilot:
        ...
        menu = app.query_one(CommandMenu)
        assert [item.name for item in menu.items] == ["model"]
...
@pytest.mark.asyncio
async def test_skill_filters_and_tab_completes_selected_skill(tmp_path: Path) -> None:
    del tmp_path
    app = CommandApp()
    async with app.run_test() as pilot:
        prompt = app.query_one(ChatInput)
        ...
        menu = app.query_one(CommandMenu)
        assert [item.name for item in menu.items] == ["code-review"]
        await pilot.press("tab")
        await pilot.pause()

        assert prompt.text == "$code-review "
        assert prompt.has_focus
        assert not menu.is_open


@pytest.mark.asyncio
async def test_skill_enter_submits_and_nonleading_dollar_does_not_open_menu(
    tmp_path: Path,
) -> None:
    del tmp_path
    app = CommandApp()
    async with app.run_test() as pilot:
        ...
        await pilot.press("$")
        await pilot.pause()
        assert len(app.query_one(CommandMenu).items) == 2

        await pilot.press("enter")
        await pilot.pause()
        assert app.submitted == ["$code-review"]

        prompt.insert("use $test-runner")
        await pilot.pause()
