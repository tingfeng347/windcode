...
from windcode.tui.widgets import ChatInput, RewindSelector, SessionSelector
...
@pytest.mark.asyncio
async def test_rewind_selector_rewinds_selected_history_node(tmp_path: Path) -> None:
    ...
    async with Windcode.open(state_root=state) as client:
        ...
        await client.start_run(RunRequest("历史任务", tmp_path, session_id="session")).result()
    ...
    async with app.run_test() as pilot:
        ...
        selector = app.query_one(RewindSelector)
        assert selector.has_focus
        ...
        assert app.rewind_selector is None
        assert app.client.load_session_messages("session") == ()
        assert chat_input.text == "历史任务"
        assert chat_input.has_focus
