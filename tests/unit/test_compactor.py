...
@pytest.mark.asyncio
async def test_discards_orphan_tool_result_before_summary_and_after_compaction() -> None:
    orphan = Message(Role.TOOL, (ToolResultBlock("orphan", "search", "result"),))
    messages = (orphan, Message(Role.USER, (TextBlock("keep"),)))
    transport = FakeTransport(valid_checkpoint())
    ...
    assert orphan not in transport.request.messages
