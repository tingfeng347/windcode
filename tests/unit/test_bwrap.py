...
def test_windows_reports_secure_degradation_without_probing_bwrap() -> None:
    status = detect_bubblewrap(platform="nt")

    assert not status.available
    assert status.executable is None
    assert "Windows" in (status.warning or "")
    assert "explicit approval" in (status.warning or "")
