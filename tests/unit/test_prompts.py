...
def test_prompt_orders_instructions_and_requires_actual_verification(tmp_path: Path) -> None:
    ...
    assert "可使用 GitHub 风格 Markdown" in prompt
    assert "列表、表格、引用、强调、行内代码和围栏代码块" in prompt
...
def test_prompt_describes_dynamic_skill_loading_policy(tmp_path: Path) -> None:
    prompt = build_system_prompt(
        workspace=tmp_path,
        permission_mode=PermissionMode.DEFAULT,
        instructions=(),
        tools=create_builtin_registry(),
        skills=(SkillSearchResult("skill:review", "review", "Review code", "user:x", None),),
    )

    assert "$review: Review code" in prompt
    assert "直接调用 load_skill" in prompt
    assert "最多调用一次 search_skills" in prompt
    assert "无需再次调用 load_skill" in prompt
...
def test_prompt_routes_explicit_memory_queries_to_memory_tools(tmp_path: Path) -> None:
    ...
    assert "根据完整对话上下文判断用户是否要删除或忘记某条记忆" in enabled
    assert "包括承接上文对象的简短确认" in enabled
