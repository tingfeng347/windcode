...
async def test_memory_write_activates_conflicting_explicit_non_sop(tmp_path: Path) -> None:
    workspace = tmp_path / "project"
    workspace.mkdir()
    service = MemoryService(tmp_path / "state", workspace)
    ...
    tool = MemoryWriteTool(
        ...
        observe,
        max_chars=4_000,
        user_prompt="记住一条经验: 使用新的验证方法",
        source=MemorySource("session", "run"),
        model=_model(),
    )
    result = await tool.execute(
        context(workspace),
        MemoryWriteInput(content="使用新的验证方法。", kind=MemoryKind.EXPERIENCE),
    )

    assert result.data["status"] == MemoryStatus.ACTIVE.value
    record = await service.store.get(str(result.data["memory_id"]))
    assert existing.memory_id in record.conflicts_with
