...
@pytest.mark.asyncio
async def test_glob_accepts_absolute_patterns_inside_workspace(tmp_path: Path) -> None:
    (tmp_path / "a.py").write_text("")
    result = await GlobTool().execute(
        ToolContext(tmp_path, "run", lambda: False),
        GlobInput(pattern=str(tmp_path / "*.py")),
    )
    assert result.output == "a.py"


@pytest.mark.asyncio
async def test_glob_rejects_absolute_patterns_outside_workspace(tmp_path: Path) -> None:
    result = await GlobTool().execute(
        ToolContext(tmp_path, "run", lambda: False),
        GlobInput(pattern="/tmp/*.py"),
    )
    assert result.is_error
    assert result.data["error"] == "pattern_outside_workspace"
