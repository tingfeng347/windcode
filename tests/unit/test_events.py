...
def test_event_serialization_normalizes_exception_values() -> None:
    event = ToolFinished(
        ...
        run_id="run",
        turn=1,
        result=ToolResult(output="failed", is_error=True, data={"cause": ValueError("bad")}),
    )

    payload = event_to_dict(event)

    json.dumps(payload)
    assert payload["result"]["data"]["cause"] == {
        "type": "ValueError",
        "message": "bad",
    }


def test_approval_event_round_trip_preserves_policy_context() -> None:
    event = ApprovalRequested(
        ...
        command_actions=({"program": "git", "action": "status"},),
        cwd="/tmp/project",
        network=True,
        sandbox_backend="bubblewrap",
        sandbox_preset="workspace_write",
        escalation_reason="sandbox unavailable",
        proposed_rule={"command": "git status"},
    )
