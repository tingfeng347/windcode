...
@pytest.mark.asyncio
async def test_manual_cancellation_is_not_mapped_to_provider_failure() -> None:
    async def cancelled(_request: ModelRequest) -> AsyncIterator[Mapping[str, object]]:
        raise asyncio.CancelledError
        yield {}

    transport = OpenAICompatibleTransport(
        api_key="test", base_url="http://localhost/v1", chunk_factory=cancelled
    )
