...
impl ProfileLock {
    fn acquire(profile: &str) -> Result<Self, String> {
        let name = wide(format!("Local\\Windcode.Sandbox.Profile.{profile}"));
        let handle = OwnedHandle(unsafe { CreateMutexW(null(), 0, name.as_ptr()) });
        if handle.0.is_null() {
        ...
        if result != WAIT_OBJECT_0 && result != WAIT_ABANDONED {
            return Err(format!("wait for sandbox profile lock failed: {result}"));
        }
        Ok(Self { handle })
    }
}
...
fn acl_lease_path(profile: &str) -> PathBuf {
    state_dir()
        .join("acl-leases")
...
fn revoke_acl(path: &Path, sid: &str) -> Result<(), String> {
    ...
    let output = Command::new("icacls.exe")
        .arg(path)
        .args(["/remove:g", &principal, "/T", "/C", "/Q"])
        .output()
        .map_err(|error| format!("failed to start icacls for {}: {error}", path.display()))?;
    if !output.status.success() {
        return Err(format!(
            "workspace ACL cleanup failed for {}: {}",
            path.display(),
            String::from_utf8_lossy(&output.stderr).trim()
        ));
    }
...
fn cleanup_acls(paths: &[PathBuf], sid: &str) -> Result<(), String> {
    let failures = paths
        .iter()
        .filter_map(|path| revoke_acl(path, sid).err())
        .collect::<Vec<_>>();
...
fn read_acl_lease(profile: &str) -> Result<Vec<PathBuf>, String> {
    let path = acl_lease_path(profile);
    if !path.exists() {
        return Ok(Vec::new());
    }
    let raw = fs::read(&path).map_err(|error| format!("read ACL lease: {error}"))?;
    let values: Vec<String> =
        serde_json::from_slice(&raw).map_err(|error| format!("parse ACL lease: {error}"))?;
    Ok(values.into_iter().map(PathBuf::from).collect())
}

fn write_acl_lease(profile: &str, paths: &[PathBuf]) -> Result<(), String> {
    let target = acl_lease_path(profile);
    let parent = target
        .parent()
        .ok_or_else(|| "ACL lease path has no parent".to_string())?;
    fs::create_dir_all(parent).map_err(|error| format!("create ACL lease directory: {error}"))?;
    let values = paths
        .iter()
    ...
    if temporary.exists() {
        fs::remove_file(&temporary).map_err(|error| format!("reset ACL lease: {error}"))?;
    }
    fs::write(&temporary, encoded).map_err(|error| format!("write ACL lease: {error}"))?;
    fs::rename(&temporary, &target).map_err(|error| format!("commit ACL lease: {error}"))
}

fn clear_acl_lease(profile: &str, sid: &str) -> Result<(), String> {
    let paths = read_acl_lease(profile)?;
    if paths.is_empty() {
        fs::remove_file(acl_lease_path(profile))
            .or_else(|error| {
            ...
            .map_err(|error| format!("remove empty ACL lease: {error}"))?;
        return Ok(());
    }
    cleanup_acls(&paths, sid)?;
    fs::remove_file(acl_lease_path(profile))
        .map_err(|error| format!("remove ACL lease: {error}"))
}
...
pub fn run(request: RunRequest) -> Result<u32, String> {
    ...
    let _profile_lock = ProfileLock::acquire(&name)?;
    let sid = derive_profile_sid(&name)?;
    let sid_text = sid_string(sid.0)?;
    clear_acl_lease(&name, &sid_text)?;
    let mut granted_paths = vec![workspace.clone()];
    let mut writable_roots = Vec::new();
    for root in &request.writable_roots {
        let root = PathBuf::from(root)
            .canonicalize()
            .map_err(|error| format!("invalid writable root {root}: {error}"))?;
        if !granted_paths.contains(&root) {
            granted_paths.push(root.clone());
        }
        writable_roots.push(root);
    }
    let temp = state_dir().join("tmp").join(std::process::id().to_string());
    if temp.exists() {
        fs::remove_dir_all(&temp).map_err(|error| format!("reset sandbox temp: {error}"))?;
    }
    fs::create_dir_all(&temp).map_err(|error| format!("create sandbox temp: {error}"))?;
    granted_paths.push(temp.clone());
    write_acl_lease(&name, &granted_paths)?;
    ...
    if let Err(error) = grant_result {
        if cleanup_acls(&granted_paths, &sid_text).is_ok() {
            let _ = fs::remove_file(acl_lease_path(&name));
        }
    ...
    let result = unsafe { launch(&request, sid.0) };
    let cleanup = cleanup_acls(&granted_paths, &sid_text);
    if cleanup.is_ok() {
        let _ = fs::remove_file(acl_lease_path(&name));
    }
    let _ = fs::remove_dir_all(&temp);
