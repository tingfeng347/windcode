...
At execution time the helper is invoked by the Python backend as:

```powershell
windcode-sandbox.exe run --workspace <dir> --cwd <dir> --preset <read_only|workspace_write> `
  --parent-pid <pid> [--writable-root <dir>]... [--network] -- <command>...
```

Every runner-side failure and denial prints `windcode-sandbox:` to stderr and exits non-zero
(77 for denials, 2 for invalid arguments) so the backend can classify it.
